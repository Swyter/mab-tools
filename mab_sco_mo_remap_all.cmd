...
for %%f in (scn_*.sco) do (
    ...
    echo. && echo ** 2^) reindx && mab_sco_unpacked_reindex.exe  --dont-reindex  --remappingfile mab_sco_mo_remap.txt  _tmp_unpack_folder             && rem swy: use our very own mab_sco_mo_remap.txt file to search for replacements, use that coupled with the data from ../../scene_props.txt to tweak _tmp_unpack_folder/mission_objects.json and do the replacements. You can add --scenepropstxt scene_props.txt to make it look up that file somewhere else when not in a SceneObj folder, remove --dont-reindex to make it update the IDs/numbers after the rename.
